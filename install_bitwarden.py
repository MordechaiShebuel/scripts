...
f'        vaultwarden/server:latest" \\{command_user}'
''
'    eend \\$?'
'}'
''
...
def create_dedicated_user():
    ...
    check_user_cmd = (
    f'if ! id "{USER}" >/dev/null 2>&1; then'
    '   echo "→ Creating system user \'{USER}\' ..."'
    '   if command -v useradd >/dev/null 2>&1; then'
    f'       useradd -r -s /sbin/nologin -d "{DATA_DIR}" -m "{USER}"'
    '    else'
    f'        adduser -S -H -s /sbin/nologin -D "{USER}"'
    '    fi'
    'fi'
...
def setup_namespaces():
    ...
    namespace_cmd = (f'if ! grep -q "^{USER}:" /etc/subuid 2>/dev/null; then'
...
# 4. Data directory & permissions
    permissions_cmd = (f'mkdir -p "{DATA_DIR}/data"'
...
def generate_token():
    ...
    generate_token_cmd = (f'if [[ ! -f "{ADMIN_TOKEN_FILE}" ]]; then'
